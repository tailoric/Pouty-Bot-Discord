...
class Thread(commands.Cog):
    ...
    async def thread(self, ctx, *, topic):
        ...
        await self.bot.db.execute("""
            INSERT INTO thread_channels (invocation_channel_id, invocation_message_id, thread_channel_id) VALUES ($1, $2, $3)
        """, ctx.channel.id, thread_start.id, disc_channel.id)
    ...
    async def thread_close(self, ctx, thread: discord.TextChannel):
        ...
        """
        thread_info = await self.bot.db.fetchrow("SELECT thread_channel_id, invocation_channel_id, invocation_message_id FROM thread_channels WHERE thread_channel_id = $1", thread.id)
        if not thread_info:
        ...
        if thread_channel:
            ...
            await self.delete_thread(thread_channel, message)
    ...
    async def delete_thread(self, thread_channel, thread_message):
